...
One of our contributions is the online finger knuckle identification. In this kind of situation, we choose the Residual Feature Network (RFNet) \cite{liu2020contactless} as our feature extraction backbone, because the model not only is lightweight enough, but it achieves state-of-the-art performance on the palmprint dataset. Meanwhile, the paper \cite{liu2020contactless} uses the soft-shifted triplet loss function, called SSTL to train the model and matching two features for dealing with shifting problem. However, in generally, feature maps of the same class will not only just shift along two axes, but also will have local deformable transformation with rotation. For solving it, we propose a new loss and also a new matching method, called rotation invariant triplet loss function (RITL). With the RITL, the feature maps can be translated along the x-axis and y-axis, and can be rotated clockwise and counterclockwise. Then we will get the minimal value after translation and rotation as the similarity scores.
